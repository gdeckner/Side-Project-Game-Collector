    ...
    public class GameInfoTest : DatabaseTest
    {
        ...
        public override void Setup()
        {
            ...
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                ...
                cmd.CommandText = @"insert into Games(game_id, game_name, rating_id, platform_id, cover_id, genre_id, franchise_id, game_description) values(200, 'CW 01', @ratingId, 0, 0, 0, 0, 'Coder wars the start!')";
        ...
        public void PullGameInfoTest()
        {
            ...
            Assert.AreEqual(0, test.genreID);
        }
        [TestMethod]
        public void PushGameInfoTest()
        {
            GameInfo test = new GameInfo();
            dao.PushGameInfo(999, "MechaCoder", "Enter the weapon that will change Coder Wars forever", 0, 0, 0,0,rtingID);
            using (SqlConnection connection = new SqlConnection(ConnectionString))
            {
                connection.Open();
                SqlCommand cmd = connection.CreateCommand();
                ...
                while(reader.Read())
                {
                    ...
                    test.genreID = (int)reader["genre_id"];
                    test.platformID = (int)reader["platform_id"];
        ...
        {
            dao.PushGameInfo(999, "CW Mecha", "Enter the weapon that will change Coder Wars forever", 0, 0, 0, 0,rtingID);
            GameInfo test = new GameInfo();
            List<GameInfo> testList = new List<GameInfo>();
